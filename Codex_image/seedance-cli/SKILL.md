---
name: seedance-cli
description: Generate images and videos independently through the official Dreamina CLI, including Dreamina image generation with Image 4.0 by default and Image 5.0Pro for explicit maximum-quality requests, plus Seedance 2.0 Fast VIP at 720p by default for supported video workflows. Use for JiMeng/Dreamina/Seedance generation, login, credits, sessions, task queries, and downloads without Gemini or GPT fallback.
---
...
- For video commands that support `--model_version`, use Seedance 2.0 Fast VIP by default: `--model_version=seedance2.0fast_vip`. The project wrapper also injects this value when no video model flag is supplied.

...
For supported video commands, default to `seedance2.0fast_vip` and `720p`. Never inject a model or resolution into `multiframe2video`, because the current CLI does not support them. Honor supported model, ratio, resolution, and duration preferences that the user explicitly includes.

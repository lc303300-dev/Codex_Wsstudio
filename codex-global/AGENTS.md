...
instructions:
  ...
  - 'Run the repository safe-update check only before writing files in the Codex_Wsstudio/Codex_image checkout. Resolve the Git root from the current or explicitly targeted repository path and run that root''s start-task.ps1 only when the file exists. Do not run or search for start-task.ps1 for pure chat, public web/GitHub search, read-only work, projectless Codex directories, or work outside this checkout. If the user explicitly requests changes to this repository and its root start-task.ps1 is missing, report the problem and stop before writing. If the script reports local changes, divergence, or an unavailable remote, report that state and do not overwrite local work.'
  - 'For a new-computer deployment request that explicitly targets Codex_Wsstudio, first resolve its checkout root, then run that root''s bootstrap-new-machine.ps1. Do not assume a projectless or unrelated current directory is the repository root. This is the shared deployment entry for Codex_image and Codex_DT.'
  - 'To synchronize this repository''s guidance and portable Codex settings into the current user global Codex home, first resolve the Codex_Wsstudio checkout root, then run its codex-global/sync-global-codex.ps1 or Sync-CodexGlobal.cmd. Never copy a machine-local config.toml directly.'
  - 'When the user asks to find an existing tool, software, GitHub repository, npm package, MCP server, Agent Skill, plugin, extension, API, integration, automation, or alternative—or when building a feature could benefit from an existing external tool—automatically read and use the globally registered Codex_Github tool-scout skill before implementing. Prefer its native feature audit and ranked V0/V1 search workflow.'

...
`config/codex/config.portable.toml` contains settings that are safe to share between computers.
`scripts/codex/setup-codex.ps1` merges those settings into the current machine's Codex config.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\codex\setup-codex.ps1
```

Preview without writing:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\codex\setup-codex.ps1 -WhatIf
```

If `cn-housing-mcp` is stored in a nonstandard location, pass its directory:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\codex\setup-codex.ps1 -CnHousingRoot D:\path\to\cn-housing-mcp
```

# Codex_IS 图片业务 Skill 通用入库标准

## 适用范围

本标准规范进入 `business-skills/` 的图片业务 Skill 包结构、事实归属、素材契约、路由、知识文件、审核和发布。它不规定槽位数量、布局、面板数、时序或具体创作方法。
...
平台不设全局槽数。每个 Skill 自行声明槽 ID、媒体类型、角色、必选性、最小/最大数量、顺序、观察要求、是否发送到生成层和描述。

通用门禁：

- `allowed_slot_ids` 与 `references` 的顺序完全一致；
- `reject_uncontracted_images` 固定为 `true`；
- `min_count <= max_count`，必选槽 `min_count >= 1`；
- 角色权威清晰，重叠时写明冲突裁决；
- 运行时不得临时增加未声明的风格图、范例图或灵感图。

## 输出、作者与执行

- 输出声明 `media_type: image`、比例确认和支持比例。
- 布局、面板数、单张/多张、方向及时序只有在确为业务硬约束时才声明。
- 作者契约必须要求提示词确认，并说明素材绑定与用户指令优先级。
...
发布收据在 staging 中生成，对排除收据后的规范化包计算 SHA-256。注册表只索引有效收据且当前包哈希一致的正式包。

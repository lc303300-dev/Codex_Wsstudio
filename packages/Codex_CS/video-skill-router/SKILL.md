...
4. 在创建项目之前，一次性确认以下三项：
   ...
   - 画幅比例；
   - 视频时长。
5. 用户确认三项设置后，加载所选 Skill 的 `contract.json`，调用 `project-pipeline` 创建项目目录和按 contract 素材槽划分的目录。
6. 向用户输出每个素材槽的绝对可点击目录链接，说明槽位用途、数量、顺序要求；不得只给相对路径或纯文本路径。
...
读取 JSON 输出中的 `project_id`、`project_dir` 和 `material_directories`。对每个素材槽都把 `source_dir` 作为用户投放源图的绝对可点击链接；`final_dir` 只用于统一生图结果回填或管线锁定最终图片。

...
6. 向用户输出每个素材槽的绝对可点击目录链接，说明槽位用途、计划数量、规则是硬性还是推荐、顺序要求；不得只给相对路径或纯文本路径。
...
读取 JSON 输出中的 `project_id`、`project_dir` 和 `material_directories`。其中 `planned_count` 已由用户时长和该 Skill 的槽位节奏规则计算。对每个素材槽都把 `source_dir` 作为用户投放源图的绝对可点击链接；`final_dir` 只用于统一生图结果回填或管线锁定最终图片。

...
凭证 schema v2 使用 `codex-cs-package-sha256-v2`：UTF-8 文本在计算包哈希前统一为无 BOM 的 LF 换行，二进制文件保持原始字节。这样同一包在 Windows、Linux、WSL 和不同 Git 换行配置下具有相同哈希。读取端可兼容历史 schema v1 凭证，但所有新发布与升级只写 schema v2。

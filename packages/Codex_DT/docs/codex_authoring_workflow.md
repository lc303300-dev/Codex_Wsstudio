...
6. Keep the source image in the mqrox asset manifest as the first ordered image reference. In this local CLI pipeline, the actual image binding is `multimodal2video --image <path>`, and the prompt references that upload as `图片1`. Do not type Web UI mention-chip forms such as `@Image 1` or `@图片1` into the CLI-facing prompt.

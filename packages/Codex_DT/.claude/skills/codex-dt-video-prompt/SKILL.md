...
6. When supplementing, newly writing, repairing, optimizing, or rewriting a video prompt, apply the local `build-seedance2-prompts` cleaning and reference-binding rules: Seedance 2.5 / 480P / all-around reference mode stay in settings, not prompt prose; remove model names, API fields, upload/tool-call wording, system-rule text, and repeated requirements from the final prompt.
7. Include camera movement, subject action, scene change, style, temporal progression, and negative constraints only when useful.
8. When the user did not specify audio, add `不生成音乐，仅生成音效。`
9. Present the optimized prompt and ask the user to choose: revise the prompt, or use `default-video-generation` with the optimized prompt and the original ordered media.

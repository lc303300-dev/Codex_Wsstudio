...
class PythonImageAdapter:
    ...
    def execute(self, request: MediaRequest, context: TaskContext) -> ProviderResult:
        ...
        if self.provider_id == "apimart-gpt-image-2":
            resolution = request.image_resolution or "4K"
            command += ["--model", self.model_id, "--workspace", str(PRIVATE_ROOT), "--size", request.image_ratio, "--resolution", resolution.lower()]
        elif self.provider_id == "google-gemini-image":
            resolution = request.image_resolution or "2K"
            command += ["--model", self.model_id, "--env-file", str(PRIVATE_ROOT / ".env"), "--aspect-ratio", request.image_ratio, "--image-size", resolution]
        try:

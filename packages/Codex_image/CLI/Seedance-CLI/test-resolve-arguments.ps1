...
foreach ($command in $videoCommands) {
    Assert-Arguments `
        ...
        -ExpectedArguments @($command, "--model_version=seedance2.0_vip", "--video_resolution=720p")
}

Assert-Arguments `
    -Name "explicit video resolution" `
    -InputArguments @("text2video", "--video_resolution=1080p") `
    -ExpectedArguments @("text2video", "--video_resolution=1080p", "--model_version=seedance2.0_vip")

Assert-Arguments `
    -Name "explicit split video resolution" `
    -InputArguments @("text2video", "--video_resolution", "4k") `
    -ExpectedArguments @("text2video", "--video_resolution", "4k", "--model_version=seedance2.0_vip")

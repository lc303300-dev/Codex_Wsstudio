...
$structureCheck = Join-Path $gitRoot "scripts\maintenance\test-project-structure.ps1"
if (-not (Test-Path -LiteralPath $structureCheck -PathType Leaf)) {
    throw "Project structure check is missing: $structureCheck"
}
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $structureCheck -RepositoryRoot $gitRoot
if ($LASTEXITCODE -ne 0) {
    throw "Project structure validation failed. Restore the standard layout before continuing."
}
